...
Produces DataFrames in the exact same schema as parse_capiq_data.py so the
dashboard can consume either source identically.
...
def _to_thousands(val):
    """Divide by 1000 to convert actual-USD to CapIQ thousands."""
    if val is None:
        return None
    return val / 1000.0
...
def _make_quarter_label(date: pd.Timestamp, fy_end_month: int) -> str:
    """Convert a quarter-end date to CapIQ-style 'YYYY FQn' label.
...
def fetch_company_financials(ticker: str, max_quarters: int = 12):
    ...
    Returns:
        (quarterly_df, cashflow_df) matching the parse_capiq_data.py schema,
    ...
    try:
        ...
        print(f"  [{ticker}] Failed to fetch EDGAR data: {e}")
        return None, None
    ...
    # ------------------------------------------------------------------
    # 3. Derived metrics (mirrors parse_capiq_data.py)
    # ------------------------------------------------------------------
    ...
    # Ensure column order matches CapIQ schema
    col_order = [
...
def fetch_peer_set(tickers: list, max_quarters: int = 12):
    ...
    Returns:
        (company_data, cashflow_data) — dicts keyed by ticker symbol,
        same structure as load_all_data() / load_cashflow_data() in dashboard.py.
    """
    company_data = {}
    cashflow_data = {}
