"""
Quarterly Metrics Dashboard — Streamlit app for equity comp analysis.

Financials are loaded live from SEC EDGAR via the sidebar (ticker symbols).
...
# Streamlit Community Cloud Secrets → os.environ (fetch_* read ATLAS_SEC_USER_AGENT at import)
try:
    _secrets = getattr(st, "secrets", None)
    if _secrets is not None and "ATLAS_SEC_USER_AGENT" in _secrets:
        os.environ.setdefault("ATLAS_SEC_USER_AGENT", str(_secrets["ATLAS_SEC_USER_AGENT"]))
except Exception:
    pass
...
def _escape_html(text) -> str:
    """Escape text for safe insertion into Streamlit markdown HTML."""
    if text is None:
        return ""
...
def render_ticker_bars(company_data_bar, market_data_bar):
    ...
    def format_item(label, price, change_pct, is_index=False):
        ...
        if change_pct is None or (isinstance(change_pct, float) and np.isnan(change_pct)):
            ...
            ch_txt = f"{abs(change_pct):.2f}%"

        item_html = (
            '<span style="display:inline-flex;align-items:center;gap:0.5rem;margin:0 2rem;white-space:nowrap;">'
            f'<span style="color:#6B8CAE;font-size:0.65rem;font-weight:700;letter-spacing:1px;">{label_safe}</span>'
            f'<span style="color:#FFFFFF;font-size:0.7rem;font-family:monospace;">{price_str}</span>'
            f'<span style="color:{color};font-size:0.65rem;font-family:monospace;">{arrow} {ch_txt}</span>'
            '</span>'
...
def _save_notes(notes: dict) -> bool:
    """Save notes dict to notes.json. Returns True on success."""
    try:
        with open(NOTES_PATH, "w") as f:
            json.dump(notes, f, indent=2)
        return True
    except OSError as e:
        logger.warning("Could not save notes to %s: %s", NOTES_PATH, e)
        return False


@st.cache_data(ttl=300)
def load_market_data():
    """Fetch live market data and normalize company names for valuation lookup."""
...
def analyze_data_freshness(company_data):
    ...
    for name, df in company_data.items():
        if df is None or df.empty or not required.issubset(df.columns):
            continue
        try:
            raw_date = df.iloc[-1]["Date"]
            latest_date = pd.Timestamp(raw_date)
        ...
        q = df.iloc[-1]["Quarter"]
        latest_quarter = "" if pd.isna(q) else str(q)
...
if not market_data.empty and market_data["Company"].duplicated().any():
    logger.warning(
        "Duplicate Company rows in market_data; lookup will use the last row per name."
    )
...
if st.session_state.quick_comp_active and st.session_state.quick_comp_valuation:
    ...
    # Map company_data keys to names matching market_data (ticker keys when live EDGAR fetch is active)
    ...
    if st.session_state.quick_comp_active:
        ...
        # No active EDGAR session: empty comp set
        for name in company_data.keys():
...
def render_info_badge(label: str, value: str):
    ...
    label_h = _escape_html(label)
    value_h = _escape_html(value)
    st.markdown(f'''
        <div style="background: #111827; border: 1px solid #1E2D45; border-radius: 6px; padding: 0.6rem 1rem;">
            <div style="color: #6B8CAE; font-size: 0.65rem; font-family: monospace; letter-spacing: 1px; text-transform: uppercase;">{label_h}</div>
            <div style="color: #4F9EFF; font-size: 0.9rem; font-family: monospace; font-weight: 600; margin-top: 0.2rem;">{value_h}</div>
        </div>
    ''', unsafe_allow_html=True)


# ---------------------------------------------------------------------------
# Sidebar: live financials from SEC EDGAR
# ---------------------------------------------------------------------------
st.sidebar.markdown("""
    <div style="padding: 1.25rem 1rem 0.75rem; border-bottom: 1px solid #1E2D45; margin-bottom: 0.5rem;">
        <div style="display: flex; align-items: center; gap: 0.5rem;">
            ...
            <span style="color: #4F9EFF; font-family: monospace; font-size: 0.7rem; font-weight: 700; letter-spacing: 2px;">SEC EDGAR</span>
        </div>
    </div>
""", unsafe_allow_html=True)
            ...
            if tickers:
                with st.spinner(f"Fetching data for {', '.join(tickers)}..."):
                    ...
                    if qc_data:
                        ...
                        st.sidebar.error(
                            "Failed to fetch data. Check ticker symbols. "
                            "SEC may block requests without a proper User-Agent: set ATLAS_SEC_USER_AGENT "
                            "to include your contact email (see https://www.sec.gov/developer)."
                        )
with col_qc2:
...
# Company selection
_report_options = company_names if company_names else []
report_companies = st.sidebar.multiselect(
    "Select Companies",
    options=_report_options,
    default=_report_options,
...
if st.sidebar.button("Export PDF", type="primary", use_container_width=True):
    if not company_names:
        st.sidebar.error("Load at least one company via SEC EDGAR in the sidebar before exporting.")
    elif not report_companies:
...
def fmt_date_iso(v):
    """Format a table date cell as YYYY-MM-DD (strings, timestamps, or datetime)."""
    if pd.isna(v):
        return ""
...
''', unsafe_allow_html=True)
    ...
    if overview.empty:
        st.info(
            "No company data to show. Use **Load tickers** in the sidebar to fetch from SEC EDGAR."
        )
    else:
...
# ---------------------------------------------------------------------------
    ...
    if not company_names:
        render_top_bar("COMPANY DEEP DIVE", company_names=None, data_source=data_source)
        st.info(
            "No companies loaded. Use **Load tickers** in the sidebar to fetch from SEC EDGAR."
        )
    else:
        ...
        <div style="margin-bottom: 1.5rem;">
            <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.25rem;">
                <div style="width: 4px; height: 28px; background: linear-gradient(180deg, #4F9EFF, #1E3A5F); border-radius: 2px;"></div>
                <h1 style="color: #FFFFFF; font-family: monospace; font-size: 1.4rem; font-weight: 700; letter-spacing: 3px; margin: 0; border: none; padding: 0;">DEEP DIVE: {_escape_html(selected)}</h1>
            </div>
            <div style="height: 1px; background: linear-gradient(90deg, #1E2D45, transparent); margin-left: 1rem;"></div>
        </div>
    ''', unsafe_allow_html=True)
        ...
        freshness = analyze_data_freshness(company_data)
        fresh_info = freshness.get(selected)
        if fresh_info is None:
            st.warning("Could not derive data freshness (needs non-empty history with Date and Quarter columns).")
            fresh_info = {
                "latest_quarter": "N/A",
        ...
        table_df = df.copy()
        # Ensure Date column is datetime before formatting (live EDGAR frames may have string dates)
        ...
        if cf_df is not None and not cf_df.empty:
            ...
            cf_table = cf_df.copy()
            if "Date" in cf_table.columns:
                cf_table["Date"] = pd.to_datetime(cf_table["Date"], errors="coerce")
            cf_table_fmt = {
                "Date": lambda v: fmt_date_iso(v),
...
''', unsafe_allow_html=True)
    ...
    if not company_names:
        ...
        # Map company_data keys to market_data company names (ticker keys when live EDGAR fetch is active)
        ticker_to_company = {}
        if st.session_state.quick_comp_active:
            for ticker in st.session_state.quick_comp_tickers:
                cleaned = _clean_company_name(ticker)
            ...
            # Inactive session: mapping only for any keys present (normally empty)
            for name in company_data.keys():
...
''', unsafe_allow_html=True)
    ...
    <div style="background: #111827; border: 1px solid #1E2D45; border-left: 3px solid #4F9EFF; border-radius: 6px; padding: 0.6rem 0.75rem; margin-bottom: 1rem;">
        ...
        <br/>
        <span style="color: #FFFFFF; font-size: 0.75rem; font-family: monospace;">{_escape_html(current_key)}</span>
    </div>
''', unsafe_allow_html=True)
    ...
    if st.session_state.notes_viewing_key and st.session_state.notes_viewing_key != current_key:
        ...
        <div style="background: #111827; border: 1px solid #4d4d1a; border-left: 3px solid #FCD34D; border-radius: 6px; padding: 0.6rem 0.75rem; margin-bottom: 1rem;">
            ...
            <br/>
            <span style="color: #FFFFFF; font-size: 0.75rem; font-family: monospace;">{_escape_html(viewing_key)}</span>
        </div>
    ''', unsafe_allow_html=True)
    ...
    with col1:
        if st.button("Save Notes", type="primary", use_container_width=True):
            ...
            if _save_notes(all_notes):
                st.success("Notes saved!")
                st.rerun()
            else:
                st.error("Could not save notes. Check disk space and file permissions for notes.json.")
    ...
    with col2:
        if updated_at:
            try:
                ...
                st.markdown(f'<p style="color: #6B8CAE; font-size: 0.85rem; margin-top: 0.5rem;">Last saved: {_escape_html(formatted_time)}</p>', unsafe_allow_html=True)
            except (ValueError, TypeError):
                pass
...
''', unsafe_allow_html=True)
    ...
    if not all_notes:
        ...
        for note_key, note_data in sorted_notes:
            ...
            display_title_h = _escape_html(display_title)
            preview_h = _escape_html(preview if preview else "(empty)")
            time_str_h = _escape_html(time_str)
            note_key_h = _escape_html(note_key)
            ...
            <div style="background: {bg_color}; border: 1px solid {border_color}; border-radius: 6px; padding: 0.75rem; margin-bottom: 0.75rem;">
                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                    <span style="color: #FFFFFF; font-size: 0.9rem; font-weight: 600;">{display_title_h}</span>
                    <span style="color: #6B8CAE; font-size: 0.75rem; font-family: monospace;">{time_str_h}</span>
                </div>
                <div style="color: #6B8CAE; font-size: 0.75rem; font-family: monospace; margin-bottom: 0.5rem;">{note_key_h}</div>
                <div style="color: #9CA3AF; font-size: 0.8rem; font-style: italic;">{preview_h}</div>
            </div>
        ''', unsafe_allow_html=True)
