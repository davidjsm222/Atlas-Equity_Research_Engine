...
def _fetch_price(ticker: str) -> dict:
    ...
    try:
        ...
        logger.debug("Yahoo price fetch failed for %s: %s", ticker, e)
...
def _fetch_shares_outstanding(ticker: str, cik_map: dict) -> int | None:
    ...
    try:
        ...
        logger.debug("SEC shares outstanding fetch failed for %s: %s", ticker, e)
    return None
...
def _load_cik_map() -> dict:
    ...
    try:
        ...
        logger.warning("Could not load SEC company_tickers.json: %s", e)
        return {}
...
def fetch_single_ticker_market_data(ticker: str, cik_map: dict = None) -> dict:
    """Fetch market data for a single ticker. Used when loading comps via SEC EDGAR in the dashboard.
